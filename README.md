...
如果一个接口中有且只有一个待实现的抽象方法（接口内用default已经实现的方法可以不管），那么我们可以将匿名内部类简写成Lambda表达式：
...
进程是程序执行的实体，每一个进程都是一个程序，都有自己的内存空间。

CPU的一个核心只能同时处理一个进程，当有多个进程需要同时运行时，CPU一般通过**时间片轮调度**算法，来实现多个进程同时运行。

时间片轮调度算法：可以理解在一段时间里，第一个时间段处理线程1，第二个时间段处理线程2，.....第n个时处理线程n，第n+1个时间段处理线程1.........

但是执行不同进程会产生上下文切换，非常耗时间。

那么怎么可以实现一个进程可以执行多个任务

**线程：**

**一个进程可以有多个线程，线程是程序执行中一个单一的顺序控制流程，现在线程成为了程序执行的最小单元，各个线程之间共享内存空间（就是进程所在的内存空间），上下文切换速度高于进程。**
...
public class Main {
    public static void main(String[] args) {
       Thread thread=new Thread(()->{
           ...
           System.out.println("我正在就算1~50之间的和");
           int sum=0;
           for(int i=0;i<=50;i++){
               sum+=i;
           }
           System.out.println("计算完成sum="+sum);
       });
       thread.start();
